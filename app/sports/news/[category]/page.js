  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (mounted) setError(e.message || "Error fetching news");
      } finally {
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold" style={{ color: "#1F1F1F" }}>
          ...
          <>
            {grouped.length === 0 ? (
              <div className="p-6 bg-white rounded shadow text-center">
                <p className="text-gray-600" style={{ color: "#1F1F1F" }}>No headlines available for this category.</p>
              </div>
            ) : (
